...
class KlineAllSymbol(ParanoidModel):
    ...
    rsma = models.DecimalField(db_column="rsma", max_digits=32, decimal_places=16, null=True, default=None)
    rsma_200 = models.DecimalField(db_column="rsma_200", max_digits=32, decimal_places=16, null=True, default=None)
        ...
        if not KlineAllSymbol.objects.filter(symbol=self.symbol, close_date_time=self.close_date_time).exists():
            ...
            if self.symbol != 'BTCUSD':
                try:
                    ...
                    if result.count() == 200:
                        self.rsma_200 = result.aggregate(avg_value=models.Avg('rsma'))['avg_value']
